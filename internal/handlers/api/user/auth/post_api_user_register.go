...
func PostAPIUserRegister(as *auth.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		user, err := as.Register(r.Context(), &userCreds)
		if err != nil {
			if errors.Is(err, repoUser.ErrLoginAlreadyExists) {
				http.Error(w, http.StatusText(http.StatusConflict), http.StatusConflict)
				return
			} else {
				logger.Log.Error(repoUser.ErrMsgCouldNotCreate,
					zap.String(middleware.TraceID, traceID),
					zap.Error(err))
				http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
				return
